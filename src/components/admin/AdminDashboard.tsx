...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  const handleStatusChange = async (order: Order, newStatus: string) => {
    try {
      console.log('Updating order status:', order.id, 'from', order.status, 'to', newStatus);
      ...
      await loadOrders();
      console.log('Orders refreshed from database, sending webhook...');
                  ...
                  <div key={order.id} className="bg-gray-50 border border-gray-200 rounded-lg p-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <label className="text-sm font-medium text-navy-900">Status:</label>
                        <select
                          value={order.status}
                          onChange={(e) => handleStatusChange(order, e.target.value)}
                          className="px-3 py-2 border border-gray-300 rounded-lg text-navy-900 focus:outline-none focus:ring-2 focus:ring-navy-900 focus:border-transparent"
                        >
